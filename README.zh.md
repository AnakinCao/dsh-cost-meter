...
- **当前模型实时价格**：通过 `session.models` RPC 读取当前会话选择的模型，直接显示该模型**实时单价**（顺序：命中/未命中/输出，元/百万 token，模型名加粗）；高峰/空闲时段自动切换（**周末全天按低谷价计费**）；**在输入框切换模型后约 2.5 秒内更新**；
...
- **峰谷计价**：自动按生效日期（2026-08-17 00:00 北京时间）与峰谷时段选择单价——高峰为北京**周一至周五** 9-12、14-18（其余空闲）；**周末（周六/周日）全天按低谷（空闲）时段计价**（2026-08-22 官方公告，自北京时间 08-23 00:00 起执行）；
- **按模型计费**：宿主从会话记录（`request/header` 事件）识别每个会话实际使用的模型，按各自单价计算；
- **按会话统计**：今日费用按会话分别聚合（宿主扫描 `~/.dsh/sessions` 下 zstd 分帧会话文件），鼠标悬停可查看**每个会话的费用明细**；**子代理（subagent）任务按 `parentSession` 自动合并进主会话**（明细标注「含N子任务」，计数只算根会话），不再把主会话与子任务混在一起显示；明细里的会话显示**工作目录名**（宿主从会话元数据 `cwd` 派生，与 GUI 侧边栏一致；DSH 不持久化会话标题，无 cwd 时回退 id）；
- **跨会话去重**：DSH 子代理会话文件会携带父会话的完整事件副本，宿主按 (时间, 轮次, 步骤, 用量) 指纹全局去重，**同一批 API 请求只计一次**，避免把父会话的用量在多个子代理文件里重复计费（曾导致今日费用虚高约 5 倍，现与平台扣费记录基本一致）；
...
- **Token 类型统计**：tooltip 内含**历史会话 Token 构成**（按类型估算占比：命令输出 / git输出 / 代码级别 / 工具输出 / 用户消息 / 助手回复 / 推理过程 + 按项目分布），为 token 消耗优化提供数据依据（宿主 `/api/token-types`，10 分钟缓存，全量扫描历史会话）。
## 安装
