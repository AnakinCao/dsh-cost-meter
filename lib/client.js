window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 会话显示名：官方 title → cwd 目录名 → id 兜底；长标题截断。 */
		function sessionLabel(id, titles) {
			if (id === undefined || id === null) return "";
			const t = titles !== null && typeof titles === "object" ? titles[id] : undefined;
			if (t === undefined || t === "") return id;
			return t.length > 48 ? t.slice(0, 48) + "…" : t;
		}

		/** 今日各会话费用明细（tooltip 用，含模型标注；子代理已合并进主会话，count>1 标注子任务数）。 */
		function sessionBreakdown(today, official, now, titles) {
			if (today === null || !Array.isArray(today.sessions)) return null;
			const rows = today.sessions
				.map((s) => ({ id: s.id, count: s.count ?? 1, cny: sessionCostCny(s, official, now), model: (s.models !== undefined && s.models.length === 1) ? s.models[0] : null }))
				.filter((r) => r.cny > 0)
				.sort((a, b) => b.cny - a.cny);
			if (rows.length === 0) return null;
			const totalFiles = typeof today.files === "number" ? today.files : 0;
			const head = "今日各会话费用（估算，子任务已合并）:" + (totalFiles > rows.length ? "（共" + totalFiles + "会话文件→" + rows.length + "会话）" : "");
			return head + "\n" + rows.map((r) => "  " + sessionLabel(r.id, titles) + (r.count > 1 ? " (含" + (r.count - 1) + "子任务)" : "") + (r.model !== null ? " (" + modelLabel(r.model) + ")" : "") + ": " + formatYuan(r.cny)).join("\n");
		}
		...
		function CostMeter(props) {
			...
			const [sessionTitles, setSessionTitles] = react.useState(null);
			react.useEffect(() => {
				let alive = true;
				const readTitles = () => {
					if (typeof props.readSessionTitles !== "function") return;
					props.readSessionTitles()
						.then((map) => {
							if (alive) setSessionTitles(map);
						})
						.catch(() => {});
				};
				const readModel = () => {
			...
			const breakdown = sessionBreakdown(today, officialPricing, now, sessionTitles);
			...
			react.useLayoutEffect(() => {
				...
				const EDGE = 12;
				if (left + r.width / 2 > window.innerWidth - EDGE) left = window.innerWidth - EDGE - r.width / 2;
				if (left - r.width / 2 < EDGE) left = EDGE + r.width / 2;
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					tipPos !== null && react_jsx_runtime.jsx("span", {
						...
						"data-side": "top",
						style: { top: tipPos.top - 8, left: tipPos.x },
						role: "tooltip",
						children: [
							...
							react_jsx_runtime.jsx("div", { className: "dshCostTipSection", children: [
								...
								react_jsx_runtime.jsx(PriceTable, { table: priceTable })
							] }),
		...
		function CostMeterDockEntry(props) {
			return react_jsx_runtime.jsx(CostMeter, {
				...
				readSessionTitles: props.readSessionTitles
			});
		}
		...
		const CSS = [
			...
			"[data-dsh-cost-meter]{display:flex;gap:10px;align-items:center;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:20px;padding:4px 0 0;white-space:nowrap;flex:0 0 auto}",
			...
			".dshCostTip[data-side=top]{transform:translate(-50%,-100%)}",
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register({
				...
				inject: (sessionId) => ({
					...
					readCurrentModel: () => ctx.connection.api.sessions.models({ sessionId })
						...
						.catch(() => null),
					readSessionTitles: () => ctx.connection.api.sessions.list({})
						.then((r) => (r !== null && typeof r === "object" && r.result !== undefined ? r.result : r))
						.then((result) => (result !== null && result.ok === true && Array.isArray(result.value?.items) ? result.value.items : null))
						.then((items) => {
							if (items === null) return null;
							// 与官方 displayTitleOf 一致：title → cwd 目录名 → id 兜底
							const map = Object.create(null);
							for (const item of items) {
								if (item === null || typeof item !== "object" || item.sessionId === undefined) continue;
								let label = item.title;
								if ((label === undefined || label === "") && typeof item.cwd === "string" && item.cwd !== "") {
									label = item.cwd.replace(/[/\\]+$/, "").split(/[/\\]/).pop() ?? "";
								}
								if (label === undefined || label === "") label = item.sessionId;
								map[item.sessionId] = label;
							}
							return map;
						})
						.catch(() => null)
