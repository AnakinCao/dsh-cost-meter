window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 价格段展示（当前模型 + 时段 + 单价），如 "Flash 空闲 1.5/0.05/4.5"。
		 *  8/17 峰谷价生效后显示 空闲/高峰；生效前为统一价，不标注时段。 */
		...
		function CostMeter(props) {
			...
			const tipContent = (breakdown !== null ? breakdown + "\n\n" : "") + priceInfo + "\n\n" + "当前模型: " + effectiveModel + "（实时单价，元/百万token: 未命中/缓存命中/输出）";
