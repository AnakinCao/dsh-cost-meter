window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 水平内边距与底部 token 栏（StatsLine）一致：内容距背景边缘 calc(侧边距 + 16px)，避免文字与背景紧贴。
		const CSS = [
			"/* dsh-cost-meter: 底部状态栏费用/余额 */",
			"[data-dsh-cost-meter]{position:relative;display:flex;gap:10px;align-items:center;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:20px;padding:4px calc(var(--dsh-composer-side-clearance) + 16px) 0;white-space:nowrap;flex:0 0 auto}",
