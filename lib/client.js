window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 价格表（tooltip 展示）：官方抓取优先，回退内置。 */
		function priceTableOf(official) {
			return official !== null && typeof official === "object" && official.flash !== undefined && official.pro !== undefined
				? official
				: FALLBACK_PRICES;
		}
		...
		function CostMeter(props) {
			...
			const tipContentTitle = "当前模型: " + effectiveModel + "（实时单价，元/百万token: 命中/未命中/输出）";
		...
		const CSS = [
			...
			"/* 追加内容与原状态栏之间的分隔符（竖线 |，两侧间距一致） */",
			"div[data-slot=\"conversation.composer.dock\"] > * + [data-dsh-cost-meter]::before{content:\"|\";color:var(--dsw-alias-separator-primary);margin:0 10px;font-weight:400}",
