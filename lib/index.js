 ...
 * @returns { sessions: [{id, count, children, models, byModel, totals}], totals, since, files }
 */
	...
	for (const file of files) {
		...
		for (const frame of decompressFrames(buf)) {
			for (const line of frame.toString("utf8").split("\n")) {
				...
				// 会话元数据（首事件）：记录 id 与父会话，用于把子代理任务合并到主会话
 ...
 * @param sessions 原始会话条目（含 meta.parent）
 * @param empty 空桶工厂
 * @returns [{id, count, children, models, byModel, totals}]
 */
function mergeSubagentSessions(sessions, empty) {
	...
	for (const s of sessions) {
		...
		if (m === undefined) {
			m = { id: rootId, count: 0, children: [], models: [], byModel: Object.create(null), totals: empty() };
