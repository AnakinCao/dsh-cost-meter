 ...
 * @returns { sessions: [{id, totals}], totals, since, files }
 */
	...
	for (const file of files) {
		...
		for (const frame of decompressFrames(buf)) {
			for (const line of frame.toString("utf8").split("\n")) {
				...
				if (inWindow) {
					sessionTotals.uncachedInputTokens += buckets.uncachedInputTokens - (previous?.uncachedInputTokens ?? 0);
					sessionTotals.outputTokens += buckets.outputTokens - (previous?.outputTokens ?? 0);
					sessionTotals.cacheReadTokens += buckets.cacheReadTokens - (previous?.cacheReadTokens ?? 0);
					sessionTotals.cacheWriteTokens += buckets.cacheWriteTokens - (previous?.cacheWriteTokens ?? 0);
				}
				last = { turn: sample.turn, step: sample.step, buckets };
			}
		}
		sessions.push({ id: file.split(/[\\/]/).slice(-2)[0], totals: sessionTotals });
		for (const key of Object.keys(grand)) grand[key] += sessionTotals[key];
	}
	return { sessions, totals: grand, since, files: scanned };
}

/** 从官方定价页 HTML 解析 deepseek-v4-flash 价格（元/百万 token）。解析失败返回 null。 */
function parseOfficialPricing(html) {
	...
	const legacy = curNums.length >= 6
		? { cachedInputPerM: curNums[0], inputPerM: curNums[2], outputPerM: curNums[4] }
		: null;
	const newIdx = text.indexOf("新价格将于北京时间");
	if (newIdx < 0) return null;
	const newSeg = text.slice(newIdx, newIdx + 500);
	const offpeak = tuple([...newSeg.matchAll(/空闲时段\s*([\d.]+)元\s*([\d.]+)元\s*([\d.]+)元/g)][0]);
	const peak = tuple([...newSeg.matchAll(/高峰时段\s*([\d.]+)元\s*([\d.]+)元\s*([\d.]+)元/g)][0]);
	return valid(legacy) && valid(offpeak) && valid(peak) ? { legacy, offpeak, peak } : null;
}
