 ...
 * @returns { sessions: [{id, models, byModel, totals}], totals, since, files }
 */
function sumTodayTokens(sessionsDir) {
	...
	for (const file of files) {
		...
		sessions.push({ id: file.split(/[\\/]/).slice(-2)[0], models: Object.keys(byModel), byModel, totals: sessionTotals });
		for (const key of Object.keys(grand)) grand[key] += sessionTotals[key];
	}
	return { sessions, totals: grand, since, files: scanned };
}
...
function parseOfficialPricing(html) {
	const text = html
		...
		.replace(/&nbsp;|&#160;/g, " ")
		.replace(/\s+/g, " ");
	const tuple = (m) => m === undefined
		? null
		: { cachedInputPerM: Number(m[1]), inputPerM: Number(m[2]), outputPerM: Number(m[3]) };
	const valid = (p) => p !== null && p.cachedInputPerM > 0 && p.inputPerM > p.cachedInputPerM && p.outputPerM > p.inputPerM && p.outputPerM < 100;
	// 当前价表：数字序列 = [flash命中, pro命中, flash未命中, pro未命中, flash输出, pro输出]
	const hitIdx = text.indexOf("缓存命中");
	if (hitIdx < 0) return null;
	const curNums = [...text.slice(hitIdx, hitIdx + 300).matchAll(/(\d+(?:\.\d+)?)\s*元/g)].map((m) => Number(m[1]));
	if (curNums.length < 6) return null;
	const legacy = {
		flash: { cachedInputPerM: curNums[0], inputPerM: curNums[2], outputPerM: curNums[4] },
		pro: { cachedInputPerM: curNums[1], inputPerM: curNums[3], outputPerM: curNums[5] }
	};
	// 新价表：flash 在 pro 之前，各两组（空闲/高峰）
	const newIdx = text.indexOf("新价格将于北京时间");
	if (newIdx < 0) return null;
	const newSeg = text.slice(newIdx, newIdx + 600);
	const offpeakMatches = [...newSeg.matchAll(/空闲时段\s*([\d.]+)元\s*([\d.]+)元\s*([\d.]+)元/g)];
	const peakMatches = [...newSeg.matchAll(/高峰时段\s*([\d.]+)元\s*([\d.]+)元\s*([\d.]+)元/g)];
	if (offpeakMatches.length < 2 || peakMatches.length < 2) return null;
	const offpeak = { flash: tuple(offpeakMatches[0]), pro: tuple(offpeakMatches[1]) };
	const peak = { flash: tuple(peakMatches[0]), pro: tuple(peakMatches[1]) };
	return valid(legacy.flash) && valid(legacy.pro) && valid(offpeak.flash) && valid(offpeak.pro) && valid(peak.flash) && valid(peak.pro)
		? { flash: { legacy: legacy.flash, offpeak: offpeak.flash, peak: peak.flash }, pro: { legacy: legacy.pro, offpeak: offpeak.pro, peak: peak.pro } }
		: null;
}
