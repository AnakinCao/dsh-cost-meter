...
- **Current model live price**: reads the current session's selected model via the `session.models` RPC and shows its live unit price (order: cache hit / cache miss / output, CNY per 1M tokens, model name bold); peak/off-peak pricing switches automatically — **weekends bill at the off-peak (valley) price all day**; updates within ~2.5s after switching the model in the composer.
...
- **Per-session aggregation**: today's cost is aggregated per session (the host scans the zstd multi-frame session files under `~/.dsh/sessions`); hover to inspect each session's breakdown. **Subagent sessions are auto-merged into their root session via `parentSession`** (breakdown lines show "含N子任务", and the count only lists root sessions). Breakdown lines show the **workspace directory name** (derived host-side from the session metadata's `cwd`, matching the GUI sidebar; DSH does not persist session titles, and the raw id is the fallback).
