...
```
今日 ¥11.52 (5会话) · 会话 ¥0.08 · v4-flash 空闲 1.5/0.05/4.5 · 余额 ¥365.36
```

- **当前模型实时价格**：通过 `session.models` RPC 读取当前会话选择的模型，状态栏直接显示该模型的实时单价（未命中/缓存命中/输出，元/百万token），高峰/空闲时段自动切换；在输入框切换模型后即时更新；
